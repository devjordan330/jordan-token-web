      ...
      <div className="relative z-10">
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 backdrop-blur-xl p-4">
            <div className="bg-[#12141d] border border-white/10 p-8 rounded-[35px] max-w-sm w-full relative shadow-2xl animate-in fade-in zoom-in duration-300">
              ...
              <div className="flex flex-col border border-white/10 rounded-3xl overflow-hidden bg-white/[0.02]">
                <button onClick={connectWallet} className="flex items-center gap-4 p-5 hover:bg-white/5 transition border-b border-white/10 group">
                  <img src="https://upload.wikimedia.org/wikipedia/commons/3/36/MetaMask_Fox.svg" className="h-7 w-7 group-hover:scale-110 transition" alt="MetaMask" />
                  <span className="font-bold text-xs uppercase tracking-widest">MetaMask</span>
                </button>
                <button onClick={connectWallet} className="flex items-center gap-4 p-5 hover:bg-white/5 transition border-b border-white/10 group">
                  <img src="https://trustwallet.com/assets/images/media/assets/trust_platform.svg" className="h-7 w-7 group-hover:scale-110 transition" alt="Trust Wallet" />
                  <span className="font-bold text-xs uppercase tracking-widest">Trust Wallet</span>
                </button>
                <button className="p-5 text-[10px] font-black uppercase text-gray-500 hover:text-[#ff5c00] transition tracking-[0.2em] bg-black/20">More Wallet Options</button>
              </div>
        ...
        <main className="max-w-7xl mx-auto px-6 pt-32">
          ...
          {/* --- ONE ASSET + USDX BRACKET SECTION --- */}
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-5 w-full">
              {[
                ...
                <div key={i} className="bg-white/[0.04] p-10 rounded-[40px] text-left border border-white/5 hover:border-[#ff5c00]/40 transition-all flex flex-col justify-between group backdrop-blur-xl h-full">
                  <div>
                    <h4 className="font-black uppercase text-xl mb-5 leading-tight">{card.t}</h4>
          ...
          {/* Newsletter Section */}
          <div className="bg-white/[0.02] rounded-[60px] p-16 md:p-28 text-center my-24 border border-white/5 backdrop-blur-2xl">
